...
class Mapping:
    ...
    def pixelindexes_constantq(self,reflection):
        ...
        if self.drx:
            ...
            k=reflection[1]
            l=reflection[2] 
            #print('q value for reflection %d%d%d is: %.4f$\AA^{-1}$'%(h,k,l,q))
    ...
    def compute_datavstheta_B(self,reflection):
        if self.drx:
    ...
    def compute_datavsphi(self,reflection):
        if self.drx:
            poi=self.pixelindexes_constantq(reflection)
    ...
    def azim_profile_fit(self,plotflag=False):
        """
        ...
        for reflection in self.reflections:  
            ...
            if plotflag:
                ...
                outputdir=path+'/azim_profile_plots/'
                os.makedirs(outputdir,exist_ok=True)
                
                
                #if target==target_array[1]:
                ...
                fname=os.path.basename(self.file).split('/')[-1]
                title=f'{fname} - Reflection {reflection} - B={self.B}mT'
                ax.set_title(title)
                ax.legend()
                #plt.show() 
...
class BatchAzimProfileExtraction():
    ...
    def run(self,plot=True,r2_threshold=0.85):
        ...
        for file in self.h5_filelist:
            try:
                ...
                map=Mapping(file,self.cif,self.reflections,threshold=self.threshold,binning=self.binning)
                samplename=map.title;Bstring=map.B
        ...
        if plot:
            ...
            for hkl in reflections:
                ...
                for i in range(3):
                    ax[i].legend()
            ax[0].set_ylabel('Gaussian Peak Position')
            ax[1].set_ylabel('Gaussian Peak Sigma')
            ax[2].set_ylabel('Gaussian Area')
