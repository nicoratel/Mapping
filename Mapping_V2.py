...
class Mapping:
    ...
    def export2sasview(self,maskfile):
    ...
    def pyFAI_extract_azimprofiles(self,qvalue):
        ...
        chi,I=ai.integrate_radial(self.data, 540, radial_range=(qvalue*(1-self.threshold), qvalue*(1+self.threshold)), radial_unit="q_A^-1",method=("no", "histogram", "cython"))
        return chi,I
    ...
    def plot_azim_profiles(self,plotphi=False):
        if self.drx:
            ...
            for reflection in self.reflections:
                ...
                subplt.set_xlabel('Angle (°)')
                subplt.set_ylabel('Intensity')
                subplt.legend()
                i+=1
            plt.legend()    
            plt.tight_layout()
    ...
    def P2(self,x, y0,I, mean, sigma,slope):
        return (1/2)*(3*np.cos(x)*np.cos(x)-1)*self.gaussian(x, y0,I, mean, sigma,slope)*np.sin(x)
    
    def P2_nobckgd(self,x,I, mean, sigma):
        return np.abs((1/2)*(3*np.cos(x)*np.cos(x)-1)*(1/I)*self.gaussian_nobckgd(x, I, mean, sigma)*np.sin(x))
    ...
    def azim_profile_fit(self,plotflag=False,printResults=False,method=None):
        """
        ...
        if self.drx: 
            for reflection in self.reflections:  
                ...
                try:
                    ...
                    #S , error = quad(self.P2,0,np.pi,args=(y0,I,mean*np.pi/180,sigma*np.pi/180,slope*np.pi/180))
                    S , error = quad(self.P2_nobckgd,0,np.pi,args=(I,mean*np.pi/180,sigma*np.pi/180))
                except:
                    y0=np.nan
                    I=np.nan
                    mean=np.nan
                    ...
                    S=np.nan
                    fitflag=False
                results[str(reflection)]=[y0,I,mean,sigma,slope,S,r_squared] 
            ...
            if printResults:
                for reflection in self.reflections:
                    print(f'Reflection{reflection}: x0={results[str(reflection)][2]:.1f}, sigma={results[str(reflection)][3]:.1f},slope={results[str(reflection)][4]:.1f},S={results[str(qvalue)][5]:.2f},R²={results[str(reflection)][6]:.2f}')  
        else:# saxs case
            for qvalue in self.qvalues:  
                if method=='pyFAI':
                ...
                try:
                    ...
                    I=params[1]
                    mean=params[2]
                    if mean<0: #may happen when using pyFAI method
                        mean+=180    
                    sigma=params[3]
                    slope=params[4]
                    # Calculate R² (coefficient of determination)
                    ...
                    r_squared=np.nan
                    fitflag=False
                results[str(qvalue)]=[y0,I,mean,sigma,slope,S,r_squared] 
...
class BatchAzimProfileExtraction():
    def __init__(self, path, cif=None, reflections:np.ndarray=None,qvalues=None,instrument='ID02',file_filter='*_waxs*_raw.h5',threshold=0.0001,binning=2,plotflag=False):
        """
    ...
    def plot_save_azim_profiles_vs_B(self,prefix='WAXS',method=None):
        ...
        for file in self.h5_filelist:
            map = Mapping(file, cif_file=self.cif, reflections=self.reflections,qvalues= self.qvalues, instrument=self.instrument,threshold=self.threshold, binning=self.binning)
    ...
    def fit_azimprofiles(self,plot=True,r2_threshold=0.85,prefix='WAXS',method=None):
        ...
        for file in self.h5_filelist:
            ...
            if method=='pyFAI':
                results=map.azim_profile_fit(plotflag=self.plotflag,method='pyFAI')
            else:
            ...
            if self.drx:                
                for reflection in self.reflections:
                    ...
                    slope = results[str(reflection)][4]
                    S= results[str(reflection)][5]
                    r_squared=results[str(reflection)][6]
                    self.batch_results.append([filename,samplename,Bstring,reflection,background,I,position,sigma,slope,S, r_squared])
            else:
                for qvalue in self.qvalues:
                    background= results[str(qvalue)][0]
                    I=results[str(qvalue)][1]
                    position= results[str(qvalue)][2]
                    sigma =results[str(qvalue)][3]
                    slope = results[str(qvalue)][4]
                    S= results[str(qvalue)][5]
                    r_squared=results[str(qvalue)][6]
                    self.batch_results.append([filename,samplename,Bstring,qvalue,background,I,position,sigma,slope,S, r_squared])
        ...
        if self.drx:
            self.df = pd.DataFrame(self.batch_results, columns=['File Name', 'Sample_name', 'B (mT)', 'hkl','background','I','Gaussian peak position','Gaussian sigma','Background slope','Degree of Order S','R_squared'])
        else:
            self.df = pd.DataFrame(self.batch_results, columns=['File Name', 'Sample_name', 'B (mT)', 'qvalue','background','I','Gaussian peak position','Gaussian sigma','Background slope','Degree of Order S','R_squared'])
        ...
        if plot:
            ...
            # Ensure 'hkl' is treated as tuples for reliable comparison
                ...
                for i, hkl in enumerate(reflections):
                    ...
                    ax[2].plot(subset['B (mT)'], subset['Degree of Order S'], marker=markers[i], label=f'Reflection {hkl}')
                for a in ax:
                    a.set_xlabel('B (mT)')
                    a.grid=True              
                ax[0].set_ylabel('Position (°)')
                ax[1].set_ylabel('$\\sigma$')
                ax[2].set_ylabel('Nematic order parameter S')
                # Create a single legend outside the subplots
                ...
                for i, qvalue in enumerate(qvalues):
                    ...
                    ax[0].plot(subset['B (mT)'], subset['Gaussian peak position'], marker=markers[i], label=f'q= {qvalue}$\\AA^{-1}$')               
                    ax[1].plot(subset['B (mT)'], subset['Gaussian sigma'], marker=markers[i], label=f'q= {qvalue}$\\AA^{-1}$')
                    ax[2].plot(subset['B (mT)'], subset['Degree of Order S'], marker=markers[i], label=f'q= {qvalue}$\\AA^{-1}$')
                for a in ax:
                    a.set_xlabel('B (mT)')
                    a.grid=True              
                ax[0].set_ylabel('Position (°)')
